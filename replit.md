...
*   **Authentication & Authorization**: Custom JWT-based authentication with bcrypt hashing and a Role-Based Access Control (RBAC) framework (Admin, SalesManager, SalesRep, ReadOnly, ProductDeveloper). The ProductDeveloper role provides restricted access to only the Resource Allocation page and assigned opportunities.
...
*   **Resource Allocation**: Workforce planning page showing resource assignments across opportunities. Uses `opportunity_resources` table to track user-to-opportunity assignments with role, allocation %, and date ranges. API: `GET /api/resource-allocation`, `GET /api/resource-allocation/opportunities`, `POST /api/resource-allocation`, `DELETE /api/resource-allocation/:id`. All authenticated roles can view the page; write operations require `ResourceAllocation.write` permission.
