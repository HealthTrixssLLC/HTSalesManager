#!/usr/bin/env bash
# Self-contained runner for the Opportunity-Contact External API integration tests.
...
if ! curl -sf -o /dev/null http://localhost:5000/api/health 2>/dev/null \
  ...
  npm run dev >/tmp/oc-test-server.log 2>&1 &
  STARTED_PID=$!
fi
...
for i in $(seq 1 60); do
  ...
  if [ "$i" = "60" ]; then
    ...
    tail -30 /tmp/oc-test-server.log 2>/dev/null >&2 || true
    exit 1
  fi
  sleep 1
done

npx vitest run tests/opportunity-contacts-api.test.ts --config tests/vitest.server.config.ts
